...
// jsQR CDN 동적 로드 (BarcodeDetector 미지원 브라우저용)
let jsQRLib = null;
function loadJsQR() {
  ...
  return new Promise((resolve, reject) => {
    ...
    script.onerror = () => reject(new Error("jsQR 로드 실패"));
...
export default function QRScanner({ onScan, onClose, title = "QR 스캔" }) {
  ...
  useEffect(() => {
    ...
    // BarcodeDetector 없으면 jsQR 로드
    let jsQR = jsQRLib;
    if (!detector && !jsQR) {
      loadJsQR().then((lib) => { jsQR = lib; }).catch(() => {});
    }
        ...
        {scanning && (
          <div className="absolute inset-0 flex items-center justify-center">
            ...
            <div className="relative w-64 h-64">
              <div className="absolute inset-0 rounded-3xl" style={{ boxShadow: "0 0 0 9999px rgba(0,0,0,0.4)" }} />
              ...
              <div className="absolute left-2 right-2 h-0.5 bg-emerald-400/80" style={{ animation: "qr-scan-line 2s ease-in-out infinite" }} />
            </div>
        ...
        {error && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/80 px-8">
            <div className="bg-white rounded-3xl p-6 text-center max-w-sm">
              ...
              <div className="flex gap-2">
                <button onClick={() => { setError(null); startCamera(facingMode); }}
